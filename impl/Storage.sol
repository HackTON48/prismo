...
contract Storage is InitializableOwnable, ReentrancyGuard {
    ...
    address public _MAINTAINER_; // collect maintainer fee to buy food for PRISMO

    address public _BASE_TOKEN_;
    address public _QUOTE_TOKEN_;
    address public _ORACLE_;
    bool public _BASE_TOKEN_IS_VAULT;
    bool public _QUOTE_TOKEN_IS_VAULT;
    ...
    function _checkPRISMOParameters() internal view returns (uint256) {
    ...
    function getBaseCapitalBalanceOf(address lp) public view returns (uint256) {
        return IPRISMOLpToken(_BASE_CAPITAL_TOKEN_).balanceOf(lp);
    }

    function getTotalBaseCapital() public view returns (uint256) {
        return IPRISMOLpToken(_BASE_CAPITAL_TOKEN_).totalSupply();
    }

    function getQuoteCapitalBalanceOf(address lp) public view returns (uint256) {
        return IPRISMOLpToken(_QUOTE_CAPITAL_TOKEN_).balanceOf(lp);
    }

    function getTotalQuoteCapital() public view returns (uint256) {
        return IPRISMOLpToken(_QUOTE_CAPITAL_TOKEN_).totalSupply();
    }
