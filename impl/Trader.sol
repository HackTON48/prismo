...
import {IPRISMOCallee} from "../intf/IPRISMOCallee.sol";
...
contract Trader is Storage, Pricing, Settlement {
    ...
    function sellBaseToken(
        ...
        if (data.length > 0) {
            IPRISMOCallee(msg.sender).prismoCall(false, amount, receiveQuote, data);
        }
    ...
    function buyBaseToken(
        ...
        if (data.length > 0) {
            IPRISMOCallee(msg.sender).prismoCall(true, amount, payQuote, data);
        }
