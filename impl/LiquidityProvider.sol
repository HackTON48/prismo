...
import {PRISMOMath} from "../lib/PRISMOMath.sol";
...
contract LiquidityProvider is Storage, Pricing, Settlement {
    ...
    returns (uint256)
    {
        ...
        if (_QUOTE_TOKEN_IS_VAULT) {
            amount = IERC4626(_QUOTE_TOKEN_).convertToShares(amount);
        }
        ...
        amount = _quoteTokenTransferIn(msg.sender, amount);
        ...
        emit Deposit(msg.sender, to, false, originAmount, capital);
        return capital;
    }
    ...
    returns (uint256)
    {
        ...
        if (_BASE_TOKEN_IS_VAULT) {
            amount = IERC4626(_BASE_TOKEN_).convertToShares(amount);
        }
        ...
        uint256 originAmount = amount;
        amount = _baseTokenTransferIn(msg.sender, amount);
        ...
        emit Deposit(msg.sender, to, true, originAmount, capital);
        return capital;
    }
    ...
    function _mintBaseCapital(address user, uint256 amount) internal {
        IPRISMOLpToken(_BASE_CAPITAL_TOKEN_).mint(user, amount);
    }

    function _mintQuoteCapital(address user, uint256 amount) internal {
        IPRISMOLpToken(_QUOTE_CAPITAL_TOKEN_).mint(user, amount);
    }

    function _burnBaseCapital(address user, uint256 amount) internal {
        IPRISMOLpToken(_BASE_CAPITAL_TOKEN_).burn(user, amount);
    }

    function _burnQuoteCapital(address user, uint256 amount) internal {
        IPRISMOLpToken(_QUOTE_CAPITAL_TOKEN_).burn(user, amount);
    }
    ...
    function getWithdrawQuotePenalty(uint256 amount) public view returns (uint256 penalty) {
        require(amount <= _QUOTE_BALANCE_, "PRISMO_QUOTE_BALANCE_NOT_ENOUGH");
        if (_R_STATUS_ == Types.RStatus.BELOW_ONE) {
            ...
            uint256 targetQuote = PRISMOMath._SolveQuadraticFunctionForTarget(
                _QUOTE_BALANCE_,
                _K_,
                fairAmount
            );
            // if amount = _QUOTE_BALANCE_, div error
            uint256 targetQuoteWithWithdraw = PRISMOMath._SolveQuadraticFunctionForTarget(
    ...
    function getWithdrawBasePenalty(uint256 amount) public view returns (uint256 penalty) {
        require(amount <= _BASE_BALANCE_, "PRISMO_BASE_BALANCE_NOT_ENOUGH");
        if (_R_STATUS_ == Types.RStatus.ABOVE_ONE) {
            uint256 spareQuote = _QUOTE_BALANCE_.sub(_TARGET_QUOTE_TOKEN_AMOUNT_);
            uint256 price = getOraclePrice();
            uint256 fairAmount = DecimalMath.divFloor(spareQuote, price);
            uint256 targetBase = PRISMOMath._SolveQuadraticFunctionForTarget(
                _BASE_BALANCE_,
                _K_,
                fairAmount
            );
            // if amount = _BASE_BALANCE_, div error
            uint256 targetBaseWithWithdraw = PRISMOMath._SolveQuadraticFunctionForTarget(
