...
import {IPRISMORewardVault, PRISMORewardVault} from "./PRISMORewardVault.sol";


contract PRISMOMine is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PRISMOs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPRISMOPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPRISMOPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PRISMOs to distribute per block.
        uint256 lastRewardBlock; // Last block number that PRISMOs distribution occurs.
        uint256 accPRISMOPerShare; // Accumulated PRISMOs per share, times 1e12. See below.
    }

    address public prismoRewardVault;
    uint256 public prismoPerBlock;
    ...
    // The block number when PRISMO mining starts.
    ...
    constructor(address _prismoToken, uint256 _startBlock) public {
        prismoRewardVault = address(new PRISMORewardVault(_prismoToken));
    ...
    function setReward(uint256 _prismoPerBlock, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        prismoPerBlock = _prismoPerBlock;
    }
    ...
    function getPendingReward(address _lpToken, address _user) external view returns (uint256) {
        ...
        uint256 accPRISMOPerShare = pool.accPRISMOPerShare;
        uint256 lpSupply = IERC20(pool.lpToken).balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 PRISMOReward = block
                .number
                .sub(pool.lastRewardBlock)
                .mul(prismoPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accPRISMOPerShare = accPRISMOPerShare.add(DecimalMath.divFloor(PRISMOReward, lpSupply));
        }
        return DecimalMath.mul(user.amount, accPRISMOPerShare).sub(user.rewardDebt);
    }
    ...
    function getAllPendingReward(address _user) external view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            totalReward = totalReward.add(
                DecimalMath.mul(user.amount, accPRISMOPerShare).sub(user.rewardDebt)
            );
        }
        return totalReward;
    }
    ...
    function getDlpMiningSpeed(address _lpToken) external view returns (uint256) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        return prismoPerBlock.mul(pool.allocPoint).div(totalAllocPoint);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 PRISMOReward = block
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        pool.accPRISMOPerShare = pool.accPRISMOPerShare.add(DecimalMath.divFloor(PRISMOReward, lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        if (user.amount > 0) {
            uint256 pending = DecimalMath.mul(user.amount, pool.accPRISMOPerShare).sub(
                user.rewardDebt
            );
            safePRISMOTransfer(msg.sender, pending);
        }
        IERC20(pool.lpToken).safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = DecimalMath.mul(user.amount, pool.accPRISMOPerShare);
    ...
    function withdraw(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        uint256 pending = DecimalMath.mul(user.amount, pool.accPRISMOPerShare).sub(user.rewardDebt);
        safePRISMOTransfer(msg.sender, pending);
    ...
    function claimAll() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            updatePool(pid);
            pending = pending.add(
                DecimalMath.mul(user.amount, pool.accPRISMOPerShare).sub(user.rewardDebt)
            );
            user.rewardDebt = DecimalMath.mul(user.amount, pool.accPRISMOPerShare);
        }
        safePRISMOTransfer(msg.sender, pending);
    ...
    // Safe PRISMO transfer function
    function safePRISMOTransfer(address _to, uint256 _amount) internal {
        IPRISMORewardVault(prismoRewardVault).reward(_to, _amount);
